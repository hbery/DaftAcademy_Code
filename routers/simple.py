...
@s_router.api_route("/method", methods=['GET', 'POST', 'PUT', 'OPTIONS', 'DELETE'])
...
@s_router.post('/register', response_model=RegisteredPerson, status_code=status.HTTP_201_CREATED)
...
@s_router.get('/patient/{patient_id}', response_model=RegisteredPerson, status_code=status.HTTP_200_OK)
async def get_patient(response: Response, patient_id: int):
	...
	return [person for person in s_router.patients if person.id == patient_id][0]
