...
def login_session(response: Response,
                  session_token: str = Depends(check_credentials)):
...
def welcome_session(request: Request, format: str = ""):
    if ("session_token" not in request.cookies.keys()) \
            or (request.cookies["session_token"] is None) \
            or (request.cookies["session_token"] not in m_router.sessions):
        raise HTTPException(
