...
def greetings(wrapped_func):
    ...
    def wrapper(*args, **kwargs):
        new_string = ' '.join([name.capitalize() for name
                               in wrapped_func(*args, **kwargs).split(' ')])
...
def is_palindrome(wrapped_func):
    ...
    def wrapper(*args, **kwargs):
        ...
        clean_string = ''.join([character.lower() for character
                                in string if character.isalnum()])
                    ...
                    if '__' not in key:
                        ...
                        for mkey in multiple_keys:
                            if mkey not in wd_keys:
                                raise ValueError
                        new_dict[key] = ' '.join([wrapped_dict[mkey] for mkey
                                                  in multiple_keys])
                else:
